...
const PostDetail = ({post}) => {
    ...
    const getContentFragment = (index, text, obj, type) => {
        ...
        switch (type) {
            ...
            case 'heading-two':
                return <h2 key={index} className="text-lg font-semibold mb-4">{modifiedText.map((item, i) =>
                    <React.Fragment key={i}>{item}</React.Fragment>)}</h2>;
            case 'paragraph':
    ...
    return (
        <div className={'bg-white shadow-lg rounded-lg lg:p-8 pb-12 mb-8'}>
            <div className={'relative overflow-hidden shadow-md mb-6'}>
                <img
                    src={post.feuturedImage.url}
                    alt={post.title}
                    className={'object-top h-full w-full rounded-t-top-lg'}
                />
            </div>
